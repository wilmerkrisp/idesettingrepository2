 ...
 * example 2
 *
 *               CommonHelperMethods.fs_service();
 *               v_1=CommonHelperMethods.s_const;
 *
 *
 *
 *
...
public final class  ${NAME}
	...
	public static < E > E deepCopyOfObject( E copied ,
	                                   Class< ? > classOfObject )
		{
		checkNotNull( copied ,"Argument should not be null." );
	 ...
	 */	
	public static <E> String getIndexesOfObjectsInCollection(Collection<E> collection, Predicate< E > filter )
	    {
	    checkNotNull( collection ,"Collection should not be null." );
	    checkNotNull( filter ,"Filter should not be null." );
	 ...
	 */	
	public static <TKey,TValue> String getIndexesOfObjectsInMap(Map<TKey,TValue> map, Predicate< TValue > filter )
		{
		checkNotNull( map ,"Map should not be null." );
		checkNotNull( filter ,"Filter should not be null." );
	 ...
	 */			
	public static <E> long getCountOfObjectsInCollection(Collection<E> collection, Predicate< E > filter )
	    {
	    checkNotNull( collection ,"Collection should not be null." );
	    checkNotNull( filter ,"Filter should not be null." );
	 ...
	 */			
	public static < TKey, TValue > long getCountOfObjectsInMap(Map<TKey,TValue> map, Predicate< TValue > filter )
	    {
	    checkNotNull( map ,"Map should not be null." );
	    checkNotNull( filter ,"Filter should not be null." );
	...
	checkCollection(c,Objects::isNull);
	 ...
	 @throws NullPointerException if argument nullable
     @throws IllegalArgumentException if argument empty (or if one collection's element is empty)
	 */
	public static <T extends Collection<E> ,E> T checkCollection(T collection,Predicate< E > filter) 
		{
...
//noNullElements(collection);
   ...
   /** Helper method for checking input args of map type or constructor in immutable colls
	...
	checkMap(m,Objects::isNull);
	 ...
	 @throws NullPointerException if argument nullable
     @throws IllegalArgumentException if argument empty (or if one map's element is empty)
	 */
	 public static < TKey, TValue >  Map<TKey,TValue> checkMap( Map<TKey,TValue> map , Predicate< TValue > filter )
		{
...
//noNullElements(collection);
    ...
    @throws IllegalStateException if argument nullable
    @throws IllegalStateException if argument empty (or if one collection's element is empty)
    */
	public static <T extends Collection<E> ,E> T checkCollectionRaiseIllegalStateException(T collection,Predicate< E > filter) 
		{
		checkState( collection!=null, "Collection should not be null." );
		checkState(!collection.isEmpty(),"Collection should not be empty." );
    ...
    @throws IllegalStateException if argument nullable
    @throws IllegalStateException if argument empty (or if one map's element is empty)
    */
	public static <TKey,TValue> Map<TKey,TValue> checkMapRaiseIllegalStateException(Map<TKey,TValue> map,Predicate< TValue > filter) 
		{
		checkState( map!=null, "Map should not be null." );
		checkState(!map.isEmpty(),"Map should not be empty." );
    ...
    if ( checkCollectionIsEmpty(c,Objects::isNull) )
	 ...
	 @throws NullPointerException if argument nullable
	*/
	public static <E> boolean checkCollectionIsWrong(Collection<E> collection,Predicate< E > filter) 
		{
    ...
    if ( checkMapIsEmpty(m,Objects::isNull) )
	 ...
	 @throws NullPointerException if argument nullable
	*/
	public static <TKey,TValue> boolean checkMapIsWrong(Map<TKey,TValue> map,Predicate< TValue > filter) 
		{
		///assert collection==null: "Argument should not be null." ; 
		...
		*/
	public static <T extends Collection<E> ,E> T checkCollectionRaiseAssertion(T collection,Predicate< E > filter) 
		{
		///assert collection==null: "Argument should not be null." ; 
		assert !collection.isEmpty() : "The validated collection is empty" ;
		assert collection.stream().noneMatch( filter ) :
		String.format("Every element of collection should not be null or empty: %s \n  Indexes of wrong elements: %s ",collection, getIndexesOfObjectsInCollection( collection, filter  ));
		return collection;
		}	
        ...
        @throws AssertionError
		*/
	public static <TKey ,TValue> Map<TKey ,TValue> checkMapRaiseAssertion(Map<TKey ,TValue> map,Predicate< TValue > filter) 
		{
		///assert collection==null: "Argument should not be null." ; 
		assert !map.isEmpty() : "The validated collection is empty" ;
		assert map.entrySet().stream().map(Map.Entry::getValue).noneMatch( filter ) :
		String.format("Every element of map should not be null or empty: %s \n  Indexes of wrong elements: %s ",map, getIndexesOfObjectsInMap( map, filter  ));
		return map;
		}		
